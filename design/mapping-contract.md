...
| The root-document guard (§12.4) | **Exact.** `config.source_suffix` is a concrete registered list, so the candidate paths are the real ones. | Best-effort in one corner: candidate suffixes are inferred from the project's discovery `include` globs, so a project whose only include glob is unreadable *and* whose root document has an exotic suffix keeps the pre-guard behaviour. | Strictly stronger, and recorded as a deliberate improvement rather than a violation. ubCode's guard would have to read the file system to match it, which is a much larger commitment than a config-time check. |
